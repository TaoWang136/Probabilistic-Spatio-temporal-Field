...
class MultimodalGenerativeCVAE(object):
    ...
    def obtain_encoded_tensors(self,
        ...
        # Encode History #
        ##################
        #print('节点历史节点历史节点历史节点历史',node_history_st.shape)
        ...
        if self.hyperparams['edge_encoding']:
            ...
            # Encode Node Edges #
            #####################
            #print('总的边的影响',len(node_edges_encoded),node_edges_encoded[0].shape,node_edges_encoded[1].shape)
            total_edge_influence,pro = self.encode_total_edge_influence(mode,
                                                                    node_edges_encoded,
        ...
        #print('状态特征变量',len(x_concat_list))
    ...
    def encode_edge(self,
        ...
        #print('邻居掩盖',len(neighbors_edge_value))
        ...
        for i, neighbor_states in enumerate(neighbors):  # Get neighbors for timestep in batch
            #print('邻居',i, len(neighbor_states),neighbor_states[0].shape)
        ...
        #print('邻居列表',len(edge_states_list),edge_states_list[0].shape)#torch.Size([3, 9, 6]) or     torch.Size([3, 9,8])   
        ...
        if self.hyperparams['edge_state_combine_method'] == 'sum':
            ...
            #print('torch.sum(neighbors_state, dim=0)',torch.sum(neighbors_state, dim=0).shape)
            combined_neighbors = torch.stack(op_applied_edge_states_list, dim=0)
            
            #print('邻居结合',combined_neighbors.shape)
        ...
        #print('加入自车节点的历史',joint_history.shape)
        ...
        last_index_per_sequence = -(first_history_indices + 1)
        #print('索引',last_index_per_sequence)
    ...
    def encode_total_edge_influence(self, mode, encoded_edges, node_history_encoder, batch_size):
        if self.hyperparams['edge_influence_combine_method'] == 'sum':
            ...
            if len(encoded_edges) == 0:
                ...
                # axis=1 because then we get size [batch_size, max_time, depth]
                #print('encoded_edges边',len(encoded_edges),encoded_edges[0].shape,encoded_edges[1].shape)
                encoded_edges = torch.stack(encoded_edges, dim=1)
                #print('encoded_edges边聚合',encoded_edges.shape,node_history_encoder.shape)
                combined_edges, pro = self.node_modules[self.node_type + '/edge_influence_encoder'](encoded_edges,
                                                                                                  node_history_encoder)
                #print('边合并编码',combined_edges.shape)
                combined_edges = F.dropout(combined_edges,
                                           p=1. - self.hyperparams['rnn_kwargs']['dropout_keep_prob'],
                                           training=(mode == ModeKeys.TRAIN))

        return combined_edges,pro
    ...
    def p_y_xz(self, mode, x, x_nr_t, y_r, n_s_t0, z_stacked, prediction_horizon,
        ...
        if self.hyperparams['dynamic'][self.node_type]['distribution']:
            #print('诊断测试1')
            y_dist, a, b, c, d = self.dynamic.integrate_distribution(a_dist, x)
            self.a=a,
            self.b=b,
            self.c=c,
            self.d=d
