...
def main():
    ...
    if not args.debug:
        ...
        model_dir = os.path.join(args.log_dir,
                                 args.site+'_models_' + time.strftime('%d_%b_%Y_%H_%M_%S', time.localtime()) + args.log_tag)
    ...
    for node_type_data_set in train_dataset:
        #print('node_type_data_set',len(node_type_data_set),node_type_data_set[0])
        ...
        if args.eval_every is not None and not args.debug and epoch % args.eval_every == 0 and epoch > 0:
            ...
            with torch.no_grad():
                ...
                for scene in tqdm(eval_scenes, desc='Sample Evaluation', ncols=80):
                    ...
                    eval_batch_errors.append(evaluation.compute_batch_statistics_train(predictions,
                                                                                 scene.dt,
                                                                                 max_hl=max_hl,
                                                                                 ph=ph,
                ...
                for scene in tqdm(eval_scenes, desc='MM Evaluation', ncols=80):
                    ...
                    eval_batch_errors_ml.append(evaluation.compute_batch_statistics_train(predictions,
                                                                                    scene.dt,
                                                                                    max_hl=max_hl,
                                                                                    ph=ph,
                                                                                    map=scene.map,
